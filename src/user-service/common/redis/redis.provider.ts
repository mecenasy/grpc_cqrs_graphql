...
export const redisProvider = {
  ...
  useFactory: async () => {
    ...
    if (!redisUri) {
      console.error('❌ [RedisProvider] REDIS_URL is missing in environment variables!');
    ...
    client.on('error', (err) => {
      console.error('❌ [RedisProvider] Client Error:', err);
    });

    try {
      await client.connect();
      console.log('✅ [RedisProvider] Connected successfully to:', redisUri.split('@')[1]);
      return client;
    } catch (error) {
